...
def make_request(url):
    req = Request('https://www.thebluealliance.com' + url)
    req.add_header('X-TBA-Auth-Key', '***REMOVED***')
...
def matrices(teams, matches):
    ...
    for match in matches:
        r1 = teams.index(int(match['alliances']['red']['team_keys'][0][3:]))
        r2 = teams.index(int(match['alliances']['red']['team_keys'][1][3:]))
        r3 = teams.index(int(match['alliances']['red']['team_keys'][2][3:]))
        b1 = teams.index(int(match['alliances']['blue']['team_keys'][0][3:]))
        b2 = teams.index(int(match['alliances']['blue']['team_keys'][1][3:]))
        b3 = teams.index(int(match['alliances']['blue']['team_keys'][2][3:]))
...
class Gui(object):
    ...
    def get_event_data(self):
        try:
            ...
            print(e)
            tkinter.messagebox.showerror('Data Unavailable',
        ...
        self.__matches.sort(key=lambda a: a['match_number'])
    ...
    def calc_oprs(self):
        ...
        """
        try:
            apr_b, c1pr_b, c2pr_b, lpr_b, tpr_b = list(zip(*self.__ranks[1:]))[3:8]
        except Exception:
            tkinter.messagebox.showerror('Data Unavailable',
        ...
        # fpr_x = [opr_x[i] - apr_x[i] - c1pr_x[i] - c2pr_x[i] - lpr_x[i] - tpr_x[i] for i in range(len(opr_x))]

        #self.__metrics = [[teams[i], opr_x[i], apr_x[i], c1pr_x[i], c2pr_x[i], lpr_x[i],
        #                  tpr_x[i], fpr_x[i], dpr_x[i]] for i in range(len(teams))]

        self.__metrics = [[teams[i], opr_x[i], 0, 0, 0, 0,
                          0, 0, dpr_x[i]] for i in range(len(teams))]
    ...
    def save(self):
        ...
        name = tkinter.filedialog.asksaveasfilename(**options)
